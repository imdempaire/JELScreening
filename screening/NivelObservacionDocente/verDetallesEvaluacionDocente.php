...
if ($id_evaluacion) {
    ...
    $sql = "SELECT dre.id_evaluacion, dc.bloque, dc.categoria, dre.resultado, dre.observaciones 
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['resultado'] == 1 ? 'Sí' : 'No'); ?></td>
                            <td><?php echo htmlspecialchars($row['observaciones']); ?></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="4">No hay detalles registrados para esta evaluación.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
