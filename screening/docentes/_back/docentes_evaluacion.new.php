...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="docentes_styles1.css">
    ...
    while ($row = $result_criterios->fetch_assoc()) {
        ...
        if ($ultimo_bloque != $row['bloque']) {
            ...
            if ($ultimo_bloque != '') {
                ...
                echo "</div>"; // Cerramos el contenido del bloque
            }

            $bloque_id++; // Incrementar el contador de bloque
            // Mostrar el título del nuevo bloque (fuera del contenedor que se oculta)
            echo "<h3 class='bloque-titulo' data-bloque='$bloque_id'>" . $row['bloque'] . "</h3>";
            // Crear el contenedor del contenido del bloque que se ocultará/mostrará
            echo "<div id='bloque-$bloque_id' class='bloque-contenido oculto'>"; // Añadimos la clase 'oculto' para controlar con CSS
