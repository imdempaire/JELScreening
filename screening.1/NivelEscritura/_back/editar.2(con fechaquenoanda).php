...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Asegúrate de que la fecha esté en el formato correcto para la base de datos
    $fecha_formateada = date('Y-m-d H:i:s', strtotime($fecha . ' ' . date('H:i:s')));

    $sql = "UPDATE evaluaciones SET nombre = ?, apellido = ?, id_estudiante = ?, grado = ?, division = ?, total_puntos = ?, fecha = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssisssii", $nombre, $apellido, $id_estudiante, $grado, $division, $total_puntos, $fecha_formateada, $id);
    
    // Depuración: Verificar los valores antes de ejecutar la consulta
    error_log("Valores a actualizar: nombre=$nombre, apellido=$apellido, id_estudiante=$id_estudiante, grado=$grado, division=$division, total_puntos=$total_puntos, fecha=$fecha_formateada, id=$id");
...
// Asegúrate de que la fecha esté en el formato correcto para el campo de entrada de tipo date
    ...
    <div class="container">
        ...
        <form action="editar.php?id=<?php echo $id; ?>" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="fecha" name="fecha" value="<?php echo htmlspecialchars($fecha_formateada); ?>" required>
            </div>
