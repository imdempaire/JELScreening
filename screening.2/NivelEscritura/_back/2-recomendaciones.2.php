<?php
$recomendaciones = [
    'tipografia' => [
        1 => "Practicar el trazado en minúscula o script por semejanza en las líneas que llevan.",
        2 => "Practicar el trazado en minúscula o script por semejanza en las líneas que llevan.",
        3 => "Trabajar con renglón pautado el afianzamiento del uso del alfabeto imprenta respetando las mayúsculas para iniciar palabras. Se puede realizar la mayúscula a color para mayor afianzamiento.Luego de afianzar la minúscula, introducir el alfabeto cursivo por familia de trazado también verbalizando la direccionalidad de los mismos. Realizar tareas de copia y dictado.",
        4 => "Pasar un texto de una tipografía a la otra. Reforzar el trazado de mayúsculas en cursiva."
    ],
    'claridad' => [
        1 => "Practicar trazos básicos de letras y números.",
        2 => "Practicar trazos básicos de letras y números.",
        3 => "Enfocarse en completar los trazos de cada letra y mejorar la claridad. Usar un modelo de referencia.",
        4 => "Refinar la precisión de trazos en escritos complejos."
    ],
    'tamaño' => [
        1 => "Practicar la escritura en líneas guía que indiquen claramente las alturas para las letras altas, bajas y chatas.",
        2 => "Practicar la escritura en líneas guía que indiquen claramente las alturas para las letras altas, bajas y chatas.",
        3 => "Refinar la consistencia del tamaño de las letras mediante la escritura de oraciones completas.",
        4 => "Mantener la coherencia en el tamaño de las letras en escritos más largos."
    ],
    'presion' => [
        1 => "Practicar la escritura con distintos niveles de presión para encontrar un punto medio cómodo.",
        2 => "Practicar la escritura con distintos niveles de presión para encontrar un punto medio cómodo.",
        3 => "Refinar la presión para lograr una escritura uniforme.",
        4 => "Mantener una presión adecuada y constante en escritos largos."
    ],
    'emplazamiento_renglon' => [
        1 => "Practicar la escritura manteniendo las letras alineadas con el renglón.",
        2 => "Practicar la escritura manteniendo las letras alineadas con el renglón.",
        3 => "Refinar la habilidad de mantener las letras dentro del renglón en párrafos más largos.",
        4 => "Mantener un emplazamiento adecuado y constante en escritos largos."
    ],
    'repeticiones' => [
        1 => "Practicar el uso de sinónimos para evitar repetir palabras.",
        2 => "Practicar el uso de sinónimos para evitar repetir palabras.",
        3 => "Variar las estructuras de las oraciones y usar pronombres para evitar repeticiones.",
        4 => "Utilizar una variedad de palabras y estructuras complejas, además de revisar y editar textos para eliminar repeticiones."
    ],
    'vocabulario' => [
        1 => "Ampliar el vocabulario mediante la lectura.",
        2 => "Ampliar el vocabulario mediante la lectura.",
        3 => "Incluir adjetivos y descripciones detalladas.",
        4 => "Utilizar un vocabulario variado y preciso."
    ],
    'conectores' => [
        1 => "Practicar la organización de ideas antes de escribir.",
        2 => "Practicar la organización de ideas antes de escribir.",
        3 => "Enfocarse en la claridad y la secuencia lógica.",
        4 => "Refinar la coherencia en textos largos."
    ],
    'longitud' => [
        1 => "Practicar la escritura de oraciones simples.",
        2 => "Practicar la escritura de oraciones simples.",
        3 => "Extender la escritura a párrafos completos.",
        4 => "Redactar textos más largos y detallados."
    ],
    'puntuacion' => [
        1 => "Practicar el uso básico de puntos y comas.",
        2 => "Practicar el uso básico de puntos y comas.",
        3 => "Enfocarse en el uso adecuado de signos de puntuación.",
        4 => "Refinar el uso de puntuación en textos complejos."
    ],
    'uso_mayuscula' => [
        1 => "Practicar el uso de mayúsculas al inicio de las oraciones y en nombres propios.",
        2 => "Practicar el uso de mayúsculas al inicio de las oraciones y en nombres propios.",
        3 => "Enfocarse en el uso correcto de mayúsculas en títulos, días de la semana, meses y festividades.",
        4 => "Refinar el uso de mayúsculas en contextos más complejos, como abreviaturas y siglas."
    ],
    'correspondencia_fonologica' => [
        1 => "Trabajar en la segmentación de palabras y sonidos.",
        2 => "Trabajar en la segmentación de palabras y sonidos.",
        3 => "Enfocarse en la corrección de errores fonológicos.",
        4 => "Refinar la fonología en palabras complejas."
    ],
    'correspondencia_ortografica' => [
        1 => "Practicar las reglas ortográficas básicas.",
        2 => "Practicar las reglas ortográficas básicas.",
        3 => "Enfocarse en reglas ortográficas más avanzadas.",
        4 => "Mantener la precisión ortográfica en escritos largos."
    ]
];
?>